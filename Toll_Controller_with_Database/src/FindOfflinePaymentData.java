...
public class FindOfflinePaymentData extends JFrame implements ActionListener {
    ...
    public FindOfflinePaymentData() {
        ...
        try {
            Conn ccc = new Conn();
            ResultSet AllData = ccc.s.executeQuery("select * from info2");
            while(AllData.next()){
                Date_Time.add(AllData.getString("Payment_gateway")); //find by
            }
    ...
    public void actionPerformed (ActionEvent ae) {
        ...
        if(ae.getSource() == SearchButton){
            //setVisible(false);
            String EmployeeSearchQuary = " select * from info2 where Payment_gateway = '"+ Date_Time.getSelectedItem()+"'";
